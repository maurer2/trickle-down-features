...
<style>
  .links {
    ...
    gap: 1rem;
    grid-auto-rows: 1fr; /*  keep same row height when wrapping; */
    list-style: none;
  ...
  .links :global(.link) {
    ...
    /* :global(&) .icon // :global() not needed here */
    & .icon {
      filter: invert() grayscale();
      font-size: 2rem;
    }

    /* &:where(:hover, :focus) .icon, // :global() needed here */
    :global(&:where(:hover, :focus)) .icon {
      filter: grayscale();
    }
  }
</style>
